    ...
    def load_programs(self):
        ...
        try:
            ...
            if not os.path.exists(config_path):
                template_path = os.path.join(script_dir, "programs_template.json")
                if os.path.exists(template_path):
                    import shutil
                    shutil.copy(template_path, config_path)
                    messagebox.showinfo("Info", "Created programs.json from template. Please configure your programs and restart the application.")
                    return []
                else:
